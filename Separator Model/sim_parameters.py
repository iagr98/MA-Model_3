...
class Settings():
    ...
    def reduce_Nx(self, dNx=10, T=100): # T anpassen, wenn T in Klasse Settings geändert wird!!!!
        ...
        self.x = np.linspace(0, self.L, self.N_x)
        self.dl = self.x[1] - self.x[0]
        print('Number of grid points reduced from '+str(self.N_x+dNx) + ' to ' +str(self.N_x) )
    ...
    def set_Nt(self, N_t):
        ...
        self.t = np.linspace(0, self.T, self.N_t)
        self.dt = self.t[1] - self.t[0]
        print('Number of time steps changed from ' + str(N_t_old) + ' to ' + str(self.N_t))
...
class Substance_System():
    ...
    def update(self,excel_file):
        ...
        self.eta_dis = hf.yaron(self.eta_c, 23e-3, self.eps_0)
